...
public class EntityEarthRupturer extends EntityMob
{
    ...
    public void onUpdate() {
        super.onUpdate();
        final List list2 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(4.0, 2.0, 4.0));
        if (list2 != null && this.getHealth() > 0.0f) {
            for (int k2 = 0; k2 < list2.size(); ++k2) {
                if (list2.get(k2) instanceof EntityPlayer && this.ticksExisted % 200 > 130) {
                    ...
                    if (((EntityPlayer) list2.get(k2)).onGround && ((EntityPlayer) list2.get(k2)).getDistanceToEntity(this) < 30.0) {
                        ((EntityPlayer) list2.get(k2)).attackEntityFrom(DamageSource.causeMobDamage(this), 8.0f);
                    }
                }
            }
        }
