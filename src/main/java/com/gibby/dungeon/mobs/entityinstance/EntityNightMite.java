...
public class EntityNightMite extends EntityMob
{
    private boolean pound;
    int counter;

    public EntityNightMite(final World par1World) {
        super(par1World);
    ...
    public void onUpdate() {
        ...
        if (this.pound && this.onGround && this.getHealth() > 0.0f && !this.worldObj.isRemote) {
            ...
            final List list2 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(3.5, 1.0, 3.5));
            if (list2 != null) {
                for (int k2 = 0; k2 < list2.size(); ++k2) {
                    if (list2.get(k2) instanceof EntityPlayer && !((EntityPlayer) list2.get(k2)).capabilities.isCreativeMode) {
                        ((EntityPlayer) list2.get(k2)).attackEntityFrom(DamageSource.outOfWorld, 5.0f);
                        final EntityLivingBase entityLivingBase = (EntityLivingBase) list2.get(k2);
                        entityLivingBase.motionY += 0.2;
                    }
                }
            }
            final EntityPoundEffect2 effect = new EntityPoundEffect2(this.worldObj, this.posX, this.posY, this.posZ);
    ...
    protected void dropFewItems(final boolean par1, final int par2) {
        this.dropItem(Dungeons.incandescentDust, 1);
    }

    public boolean getCanSpawnHere() {
        return this.worldObj.difficultySetting != EnumDifficulty.PEACEFUL && this.worldObj.checkNoEntityCollision(this.boundingBox) && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox).isEmpty() && !this.worldObj.isAnyLiquid(this.boundingBox) && this.posY < 70.0 && this.posY > 60.0;
    }
}
