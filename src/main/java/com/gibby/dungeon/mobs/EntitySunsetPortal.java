...
public class EntitySunsetPortal extends EntityThrowable
{
    boolean impacted;
    int counter;
    int stage;
    ...
    public EntitySunsetPortal(final World par1World) {
        super(par1World);
        this.stage = 0;
        this.freezeX = 0.0;
        this.freezeY = 0.0;
    ...
    public EntitySunsetPortal(final World par1World, final EntityLivingBase par2EntityLivingBase) {
    ...
    public EntitySunsetPortal(final World par1World, final double par2, final double par4, final double par6) {
    ...
    public void onUpdate() {
        ...
        if (this.impacted) {
            ...
            this.worldObj.spawnParticle("angryVillager", this.posX + randX, this.posY + randY, this.posZ + randZ, this.rand.nextGaussian() / 10.0, this.rand.nextGaussian() / 10.0, this.rand.nextGaussian() / 10.0);
            for (int i = 0; i < 40; ++i) {
                ...
                this.worldObj.spawnParticle("enchantmenttable", this.posX + rand2X, this.posY + rand2Y, this.posZ + rand2Z, -(rand2X + rand2Z / 2.0), 0.5, -(rand2Z + rand2X / 2.0));
            }
            for (int i = 0; i < 40; ++i) {
                final int rand2Y = this.rand.nextInt(10) - this.rand.nextInt(10);
                final double rand2X = this.rand.nextGaussian() - this.rand.nextGaussian();
                final double rand2Z = this.rand.nextGaussian() - this.rand.nextGaussian();
                this.worldObj.spawnParticle("flame", this.posX + rand2X, this.posY + rand2Y, this.posZ + rand2Z, -(rand2X + rand2Z / 2.0), 0.5, -(rand2Z + rand2X / 2.0));
            }
            for (int i = 0; i < 40; ++i) {
                final int rand2Y = this.rand.nextInt(10) - this.rand.nextInt(10);
                final double rand2X = this.rand.nextGaussian() - this.rand.nextGaussian();
                final double rand2Z = this.rand.nextGaussian() - this.rand.nextGaussian();
                this.worldObj.spawnParticle("driplava", this.posX + rand2X, this.posY + rand2Y, this.posZ + rand2Z, -(rand2X + rand2Z / 2.0), 0.5, -(rand2Z + rand2X / 2.0));
            }
            ++this.counter;
            ...
            if (this.counter > 40) {
                ...
                if (!this.worldObj.isRemote && this.ticksExisted % 40 == 0) {
                    switch (this.stage) {
                        ...
                        case 6: {
                            this.worldObj.setBlock(X + 1, Y + 2, Z, Dungeons.portalSunset);
                            this.worldObj.setBlock(X - 1, Y + 2, Z, Dungeons.portalSunset);
                            this.worldObj.setBlock(X, Y + 2, Z + 1, Dungeons.portalSunset);
                            this.worldObj.setBlock(X, Y + 2, Z - 1, Dungeons.portalSunset);
                            ++this.stage;
                            break;
                        }
                        case 7: {
                            this.worldObj.setBlock(X + 1, Y + 2, Z + 1, Dungeons.portalSunset);
                            this.worldObj.setBlock(X - 1, Y + 2, Z - 1, Dungeons.portalSunset);
                            this.worldObj.setBlock(X - 1, Y + 2, Z + 1, Dungeons.portalSunset);
                            this.worldObj.setBlock(X + 1, Y + 2, Z - 1, Dungeons.portalSunset);
                            ++this.stage;
