        ...
        if (par4.magicAmount() >= 25 || par3.capabilities.isCreativeMode) {
            if (par2World.isRemote) {
                for (int i = 0; i < 60; ++i) {
                    ...
                    particle = new EntityFireworkSparkFX(par2World, par3.posX + particlePositionX, par3.posY - 1.0, par3.posZ + particlePositionZ, 0.0, 0.0, 0.0, Minecraft.getMinecraft().effectRenderer);
                    particle.setRBGColorF(1.0f, 0.0f, 1.0f);
                    Minecraft.getMinecraft().effectRenderer.addEffect(particle);
    ...
    public boolean onEntitySwing(final EntityLivingBase entityLiving, final ItemStack stack) {
        if (entityLiving instanceof EntityPlayer) {
            ...
            if (par4.magicAmount() >= 3 || par3.capabilities.isCreativeMode) {
                ...
                if (!par3.worldObj.isRemote) {
                    final EntityMagirockBall ball = new EntityMagirockBall(par3.worldObj, par3);
