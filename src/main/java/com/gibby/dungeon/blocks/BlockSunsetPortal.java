...
import com.gibby.dungeon.gen.TeleporterSunset;
...
public class BlockSunsetPortal extends Block
{
    public BlockSunsetPortal() {
    ...
    public void onEntityCollidedWithBlock(final World par1World, final int par2, final int par3, final int par4, final Entity par5Entity) {
        if (par5Entity.ridingEntity == null && par5Entity.riddenByEntity == null && par5Entity instanceof EntityPlayerMP) {
            ...
            else if (player.dimension != Dungeons.sunsetDimensionId) {
                player.timeUntilPortal = 3;
                player.mcServer.getConfigurationManager().transferPlayerToDimension(player, Dungeons.sunsetDimensionId, new TeleporterSunset(mServer.worldServerForDimension(Dungeons.sunsetDimensionId)));
            }
            else {
                player.timeUntilPortal = 3;
                player.mcServer.getConfigurationManager().transferPlayerToDimension(player, 0, new TeleporterSunset(mServer.worldServerForDimension(0)));
    ...
    public void onNeighborBlockChange(final World par1World, final int par2, final int par3, final int par4, final Block par5) {
        ...
        if (par1World.getBlock(par2, i1 - 1, par4) != Blocks.stonebrick) {
            par1World.setBlockToAir(par2, par3, par4);
        }
        else {
            int j1;
            for (j1 = 1; j1 < 4 && par1World.getBlock(par2, i1 + j1, par4) == this; ++j1) {}
            if (j1 == 3 && par1World.getBlock(par2, i1 + j1, par4) == Blocks.stonebrick) {
                ...
                else if ((par1World.getBlock(par2 + b0, par3, par4 + b2) != Blocks.stonebrick || par1World.getBlock(par2 - b0, par3, par4 - b2) != this) && (par1World.getBlock(par2 - b0, par3, par4 - b2) != Blocks.stonebrick || par1World.getBlock(par2 + b0, par3, par4 + b2) != this)) {
                    par1World.setBlockToAir(par2, par3, par4);
                }
            }
        }
