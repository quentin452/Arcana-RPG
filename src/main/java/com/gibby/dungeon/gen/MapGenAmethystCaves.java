...
public class MapGenAmethystCaves extends MapGenBase
{
    ...
    private boolean isTopBlock(final Block[] data, final int index, final int x, final int y, final int z, final int chunkX, final int chunkZ) {
        ...
        else if (data[index] == Dungeons.crystalliumGrass) {
            return true;
        }
        if (this.isExceptionBiome(biome) ? (data[index] != Dungeons.crystalGrass) : (data[index] != Dungeons.crystalGrass)) {
            return false;
        }
        return true;
    }
    ...
    protected void digBlock(final Block[] data, final int index, final int x, final int y, final int z, final int chunkX, final int chunkZ, final boolean foundTop) {
        ...
        final Block filler = this.isExceptionBiome(biome) ? Dungeons.crystalliumDirt : Dungeons.crystalliumDirt;
        final Block block = data[index];
        if (block == Dungeons.amethystStone || block == filler || block == top) {
            if (y < 10) {
