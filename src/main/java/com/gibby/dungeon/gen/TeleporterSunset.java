...
public class TeleporterSunset extends Teleporter
{
    ...
    public boolean placeInExistingPortal(final Entity par1Entity, final double par2, final double par4, final double par6, final float par8) {
            ...
            for (int k2 = l - short1; k2 <= l + short1; ++k2) {
                ...
                for (int l2 = i2 - short1; l2 <= i2 + short1; ++l2) {
                    ...
                    for (int i3 = this.worldServerInstance.getActualHeight() - 1; i3 >= 0; --i3) {
                        if (this.worldServerInstance.getBlock(k2, i3, l2) == Dungeons.portalSunset) {
                            while (this.worldServerInstance.getBlock(k2, i3 - 1, l2) == Dungeons.portalSunset) {
                                --i3;
                            }
        ...
        if (d3 >= 0.0) {
            ...
            double d6 = k + 0.5;
            int j3 = -1;
            if (this.worldServerInstance.getBlock(i - 1, j, k) == Dungeons.portalSunset) {
                j3 = 2;
            }
            if (this.worldServerInstance.getBlock(i + 1, j, k) == Dungeons.portalSunset) {
                j3 = 0;
            }
            if (this.worldServerInstance.getBlock(i, j, k - 1) == Dungeons.portalSunset) {
                j3 = 3;
            }
            if (this.worldServerInstance.getBlock(i, j, k + 1) == Dungeons.portalSunset) {
                j3 = 1;
