...
public class TeleporterMidnight extends Teleporter
{
    ...
    public TeleporterMidnight(final WorldServer par1WorldServer) {
    ...
    public void placeInPortal(final Entity par1Entity, final double par2, final double par4, final double par6, final float par8) {
        if (this.worldServerInstance.provider.dimensionId == Dungeons.midnightDimensionId || this.worldServerInstance.provider.dimensionId == 0) {
            ...
            for (int l = -2; l <= 2; ++l) {
                for (int i2 = -2; i2 <= 2; ++i2) {
                    for (int j2 = -1; j2 < 3; ++j2) {
                        ...
                        this.worldServerInstance.setBlock(k2, l2, i3, flag ? Dungeons.darkBlueGlow : Blocks.air);
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(final Entity par1Entity, final double par2, final double par4, final double par6, final float par8) {
        ...
        if (this.destinationCoordinateCache.containsItem(j2)) {
            final TeleporterMidnight.PortalPosition portalposition = (TeleporterMidnight.PortalPosition)this.destinationCoordinateCache.getValueByKey(j2);
            d3 = 0.0;
            ...
            for (int k2 = l - short1; k2 <= l + short1; ++k2) {
                ...
                for (int l2 = i2 - short1; l2 <= i2 + short1; ++l2) {
                    ...
                    for (int i3 = this.worldServerInstance.getActualHeight() - 1; i3 >= 0; --i3) {
                        if (this.worldServerInstance.getBlock(k2, i3, l2) == Dungeons.midnightPortal) {
                            while (this.worldServerInstance.getBlock(k2, i3 - 1, l2) == Dungeons.midnightPortal) {
                                --i3;
                            }
        ...
        if (d3 >= 0.0) {
            ...
            double d6 = k + 0.5;
            int j3 = -1;
            if (this.worldServerInstance.getBlock(i - 1, j, k) == Dungeons.midnightPortal) {
                j3 = 2;
            }
            if (this.worldServerInstance.getBlock(i + 1, j, k) == Dungeons.midnightPortal) {
                j3 = 0;
            }
            if (this.worldServerInstance.getBlock(i, j, k - 1) == Dungeons.midnightPortal) {
                j3 = 3;
            }
            if (this.worldServerInstance.getBlock(i, j, k + 1) == Dungeons.midnightPortal) {
                j3 = 1;
    ...
    public boolean makePortal(final Entity par1Entity) {
        ...
        for (int x = 0; x < 7; ++x) {
            for (int z = 0; z < 7; ++z) {
                this.worldServerInstance.setBlock(i + x - 30, j + 17, k + z - 20, Blocks.air);
                this.worldServerInstance.setBlock(i + x - 30, j + 18, k + z - 20, Blocks.air);
            }
        }
        for (int x = 0; x < 7; ++x) {
            for (int z = 0; z < 7; ++z) {
                this.worldServerInstance.setBlock(i + x - 30, j + 15, k + z - 20, Dungeons.darkBlueGlow);
            }
        }
        for (int x = 1; x < 6; ++x) {
            for (int z = 1; z < 6; ++z) {
                this.worldServerInstance.setBlock(i + x - 30, j + 16, k + z - 20, Dungeons.darkBlueGlow);
            }
        }
        for (int x = 2; x < 5; ++x) {
            for (int z = 2; z < 5; ++z) {
                this.worldServerInstance.setBlock(i + x - 30, j + 16, k + z - 20, Dungeons.midnightPortal);
            }
        }
        return true;
    }
    ...
    public void removeStalePortalLocations(final long par1) {
        if (par1 % 100L == 0L) {
            ...
            while (iterator.hasNext()) {
                ...
                final TeleporterMidnight.PortalPosition portalposition = (TeleporterMidnight.PortalPosition)this.destinationCoordinateCache.getValueByKey(olong);
